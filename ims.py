...
server = 'KOTA\SQLEXPRESS'
databse = 'newims'
driver = '{SQL server}'

connection_string = f'DRIVER={driver};SERVER={server};DATABASE={databse};trusted_connection=yes'
...
def addproduct():
    if request.method=='POST':
        ...
        cn.execute(f"insert into product(product_name,stock,supplier_id,price) values('{productname}','{stock}','{supplierid}','{price}')")
        conn.commit()
        print('Data as been Inserted')
        return jsonify({'message':'sucessfull'})
    else:
...
def addorder():
    if request.method=='POST':
        ...
        cn.execute(f"insert into orders (product_id,customer_id,customer_email) values('{productid}','{customerid}','{customeremail}')")
        conn.commit()
        print('Data as been Inserted')
        return jsonify({'message':'sucessfull'})
    else:
...
def addsupplier():
    if request.method=='POST':
        ...
        cn.execute(f"insert into supplier (supplier_id,supplier_name,supplier_addr,supplier_email) values('{supplierid}','{suppliername}','{supplieraddr}','{supplieremail}')")
        conn.commit()
        print('Data as been Inserted')
        return jsonify({'message':'sucessfull'})
    else:
